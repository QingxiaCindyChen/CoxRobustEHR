---
title: "Various Simulations to Compare Cox Regression and Logistic Regression - Exponential (Left Truncation)"
...
survdata <- function(seed, n, error, p1, mu, sd, alpha_vec, beta_1, beta_vec, lambda) {
  ...
  error_case4 <- data %>% filter(time_error < time_left_trunc)
...
# output(seed=123, n=500, error=1, p1=0.3, mu=0.5, sd=0.4, alpha_vec=c(-1.25,1), beta_1=c(log(1),log(1.10),log(1.15),log(1.25),log(1.5),log(2)), beta_vec=c(log(2),log(2)), lambda=0.001, loopnum=5000)
...
# jpeg("res_sim1_exp_lt_l.jpeg", units="in", width=24, height=18, res=300)
...
res_fig <- function(result0, result1, result2, result3, result4) {
  ...
  ggplot(res_df, aes(x=betas, y=eval, col=Model, group=Model)) +
    ...
    #scale_x_continuous(breaks=c(log(1), log(1.1), log(1.15), log(1.25), log(1.5), log(2)), labels=c("1", "1.1", "1.15", "1.25", "1.5", "2"))
  
  # 1: angle=35, vjust=0.8, hjust=0.7, size=7; scale_x_continuous(breaks=c(log(1), log(1.1), log(1.15), log(1.25), log(1.5), log(2)), labels=c("log(1)", "log(1.1)", "log(1.15)", "log(1.25)", "log(1.5)", "log(2)")); width=0.035
  
  # 2: angle=30, vjust=0.72, size=9; width = 0.3
  
  # 3: size=9, width=0.035
}
```

```{r fig.width=9, fig.height=6}
# tiff("res_sim1_exp_lt_l.tiff", units="in", width=9, height=6, res=300)
#jpeg("res_sim1_exp_lt_l.jpeg", units="in", width=9, height=6, res=300)
res_fig(result0, result1, result2, result3, result4)
# dev.off()
```

```{r}
res_fig_nocol <- function(result0, result1, result2, result3, result4) {
  #betas <- rep(c(log(1), log(1.1), log(1.15), log(1.25), log(1.5), log(2)), 4*5)
  betas <- rep(c(1, 1.1, 1.15, 1.25, 1.5, 2), 4*5)
  #betas <- rep(c("log(1)", "log(1.1)", "log(1.15)", "log(1.25)", "log(1.5)", "log(2)"), 4*5)
  #betas <- factor(betas, levels=c("log(1)", "log(1.1)", "log(1.15)", "log(1.25)", "log(1.5)", "log(2)"))
  ...
  #ggplot(res_df, aes(x=betas, y=eval, linetype=Model, shape=Model, group=Model)) +
  ggplot(res_df, aes(x=betas, y=eval, linetype=Model, group=Model)) +
    #geom_line(size=0.4) + geom_point(size=1) +
    geom_line(size=0.4) + geom_point(size=0.7) +
    geom_errorbar(aes(ymin=lb, ymax=ub, width=0.035), linetype=1) +
    geom_hline(yintercept=0.05, linetype="dashed", color="grey") +
    scale_linetype_manual(values=c(4,1,5,3)) +
    #scale_shape_manual(values=c(15,17,16,18)) +
    facet_wrap(~ det) +
    ylim(0,1) +
    labs(x="Hazard Ratio for Exposure, z", y="Type I Error and Power *") + 
    theme_bw() + 
    theme(panel.grid.major.x = element_blank(), panel.grid.minor = element_blank(), axis.text.x = element_text(angle=35, vjust=0.8, hjust=0.7, size=7), axis.text.y = element_text(size=9), axis.title.x = element_text(size=12), axis.title.y = element_text(size=12), title = element_text(size=23), legend.position=c(0.85, 0.18), legend.key.size = unit(1, 'cm'), legend.title = element_text(size=12), strip.text = element_text(size=11)) + scale_x_continuous(breaks=c(1, 1.1, 1.15, 1.25, 1.5, 2), labels=c("1", "1.1", "1.15", "1.25", "1.5", "2"))
...
# tiff("res_sim1_exp_lt_l.tiff", units="in", width=9, height=6, res=300)
#jpeg("res_sim1_exp_lt_l.jpeg", units="in", width=9, height=6, res=300)
res_fig_nocol(result0, result1, result2, result3, result4)
#dev.off()
```
...
res1_fig <- ggplot(res_df, aes(x=betas, y=eval, col=mod)) + geom_line() + geom_point() + geom_errorbar(aes(ymin=lb, ymax=ub, width=0.03)) + geom_hline(yintercept=0.05, linetype="dashed", color="black") + labs(x="Beta Coefficient for Exposure, z", y="Type I Error/Power", title="Error Independent", color="Model") + theme_bw() + theme(panel.grid.major.x = element_blank(), panel.grid.minor = element_blank(), axis.text.x = element_text(angle=30, vjust=0.72, size=7), axis.text.y = element_text(size=7), axis.title.x = element_text(size=10), axis.title.y = element_text(size=10), title = element_text(size=12)) + scale_x_continuous(breaks=c(log(1), log(1.1), log(1.15), log(1.25), log(1.5), log(2)), labels=c("log(1)", "log(1.1)", "log(1.15)", "log(1.25)", "log(1.5)", "log(2)"))

# jpeg("explain_sim_res.jpeg", units="in", width=6, height=4, res=300)
# res1_fig
# dev.off()
```
...
#jpeg("exp_lt_large_res2.jpeg", units="in", width=6, height=4, res=300)
res2_fig
#dev.off()
```
...
bias_fig <- ggplot(bias_df, aes(x=betas, y=bias, col=error)) + geom_point(position=position_dodge(0.04)) + geom_errorbar(aes(ymin=lb, ymax=ub, width=0.1), position=position_dodge(0.04)) + geom_hline(yintercept=0, linetype="dashed", color="black") + labs(x="Beta Coefficient for Exposure, z", y="Bias", title="Bias +/- Empirical Standard Deviation of Beta Coefficient for Exposure, z, from Model 1 (Cox)", color="Error") + theme_bw() + theme(panel.grid.major.x = element_blank(), panel.grid.minor = element_blank(), axis.text.x = element_text(angle=30, vjust=0.72)) + scale_x_continuous(breaks=c(1, 2, 3, 4, 5, 6), labels=c("log(1)", "log(1.1)", "log(1.15)", "log(1.25)", "log(1.5)", "log(2)"))

bias_fig_nl <- bias_fig + theme(legend.position = "none")
bias_fig_tab <- ggarrange(bias_fig_nl, bias_tab, nrow=2, heights=c(2,1)) 

#jpeg("bias_sim1_exp_lt_l.jpeg", units="in", width=7, height=4, res=300)
bias_fig
#dev.off()
```

```{r}
result0_exp_l_lt <- result0
result1_exp_l_lt <- result1
result2_exp_l_lt <- result2
result3_exp_l_lt <- result3
result4_exp_l_lt <- result4
```

```{r warning = FALSE, message = FALSE}

CaseCount <- function(error=error, beta1=beta) {
  data <- survdata(seed=123, n=500, error=error, p1=0.3, mu=0.5, sd=0.4, alpha_vec=c(-1.25,1), beta_1=beta1, beta_vec=c(log(2),log(2)), lambda=0.001)
  n <- c(dim(data[[1]])[1], dim(data[[2]])[1], dim(data[[3]])[1], dim(data[[4]])[1], dim(data[[5]])[1])
  Number <- c(paste0("Error=", error, "n"), paste0("Complete Data, N = ", n[1]), paste0("\n Delayed Event Time Case 1, n = ", n[2]), paste0("\n Delayed Event Time Case 2, n = ", n[3]), paste0("\n Delayed Event Time Case 3, n = ", n[4]), paste0("\n Event Before EHR, n = ", n[5]))
  Rate <- c(paste0(round(n[5]/n[1],4),"Removed \n"), paste0(round(n[3]/(n[1]-n[5]),4), "misclassified \n"))
  
  cat(Rate)
}  

CaseCount(error=0, beta1=log(1))
CaseCount(error=0, beta1=log(1.1))
CaseCount(error=0, beta1=log(1.15))
CaseCount(error=0, beta1=log(1.25))
CaseCount(error=0, beta1=log(1.5))
CaseCount(error=0, beta1=log(2))

CaseCount(error=1, beta1=log(1))
CaseCount(error=1, beta1=log(1.1))
CaseCount(error=1, beta1=log(1.15))
CaseCount(error=1, beta1=log(1.25))
CaseCount(error=1, beta1=log(1.5))
CaseCount(error=1, beta1=log(2))

CaseCount(error=2, beta1=log(1))
CaseCount(error=2, beta1=log(1.1))
CaseCount(error=2, beta1=log(1.15))
CaseCount(error=2, beta1=log(1.25))
CaseCount(error=2, beta1=log(1.5))
CaseCount(error=2, beta1=log(2))

CaseCount(error=3, beta1=log(1))
CaseCount(error=3, beta1=log(1.1))
CaseCount(error=3, beta1=log(1.15))
CaseCount(error=3, beta1=log(1.25))
CaseCount(error=3, beta1=log(1.5))
CaseCount(error=3, beta1=log(2))

CaseCount(error=4, beta1=log(1))
CaseCount(error=4, beta1=log(1.1))
CaseCount(error=4, beta1=log(1.15))
CaseCount(error=4, beta1=log(1.25))
CaseCount(error=4, beta1=log(1.5))
CaseCount(error=4, beta1=log(2))
...
# debugonce(survdata); survdata(seed=123, n=500, error=0, p1=0.3, mu=0.5, sd=0.4, alpha_vec=c(-1.25,1), beta_1=log(2), beta_vec=c(log(2),log(2)), lambda=0.001)
...
```{r error0-2-run, warning = FALSE, message = FALSE, eval=rerun}
# Results
result <- output(seed=123, n=500, error=0, p1=0.3, mu=0.5, sd=0.4, alpha_vec=c(-1.25,1), beta_1=c(log(1),log(1.10),log(1.15),log(1.25),log(1.5),log(2)), beta_vec=c(log(2),log(2)), lambda=0.001, loopnum=5000)
write.csv(result, file='result02.csv')

result <- output(seed=123, n=500, error=1, p1=0.3, mu=0.5, sd=0.4, alpha_vec=c(-1.25,1), beta_1=c(log(1),log(1.10),log(1.15),log(1.25),log(1.5),log(2)), beta_vec=c(log(2),log(2)), lambda=0.001, loopnum=5000)
save.csv(result, file='result12.csv')

result <- output(seed=123, n=500, error=2, p1=0.3, mu=0.5, sd=0.4, alpha_vec=c(-1.25,1), beta_1=c(log(1),log(1.10),log(1.15),log(1.25),log(1.5),log(2)), beta_vec=c(log(2),log(2)), lambda=0.001, loopnum=5000)
save.csv(result, file='result22.csv')

result <- output(seed=123, n=500, error=3, p1=0.3, mu=0.5, sd=0.4, alpha_vec=c(-1.25,1), beta_1=c(log(1),log(1.10),log(1.15),log(1.25),log(1.5),log(2)), beta_vec=c(log(2),log(2)), lambda=0.001, loopnum=5000)
save.csv(result, file='result32.csv')

result <- output(seed=123, n=500, error=4, p1=0.3, mu=0.5, sd=0.4, alpha_vec=c(-1.25,1), beta_1=c(log(1),log(1.10),log(1.15),log(1.25),log(1.5),log(2)), beta_vec=c(log(2),log(2)), lambda=0.001, loopnum=5000)
save.csv(result, file='result42.csv')

result <- output(seed=123, n=500, error=5, p1=0.3, mu=0.5, sd=0.4, alpha_vec=c(-1.25,1), beta_1=c(log(1),log(1.10),log(1.15),log(1.25),log(1.5),log(2)), beta_vec=c(log(2),log(2)), lambda=0.001, loopnum=5000)
save.csv(result, file='result52.csv')
...
#jpeg("res_sim1_exp_lt_s.jpeg", units="in", width=24, height=18, res=300)
# leg <- get_legend(res_fig0)
# res_fig0 <- res_fig0 + theme(legend.position = "none")
# res_fig1 <- res_fig1 + theme(legend.position = "none")
# res_fig2 <- res_fig2 + theme(legend.position = "none")
...
#jpeg("exp_lt_small_res2.jpeg", units="in", width=6, height=4, res=300)
res2_fig
#dev.off()
```

```{r fig.width=9, fig.height=6}
# tiff("res_sim1_exp_lt_s.tiff", units="in", width=9, height=6, res=300)
#jpeg("res_sim1_exp_lt_s.jpeg", units="in", width=9, height=6, res=300)
res_fig(result0, result1, result2, result3, result4)
#dev.off()
```
...
#jpeg("bias_sim1_exp_lt_s.jpeg", units="in", width=7, height=4, res=300)
# bias_fig_tab
bias_fig
#dev.off()
```
...
```{r eval=FALSE}
# Plot results of original time and time with error
sim0plot <- survdata(seed=123, n=1000000, error=0, p1=0.3, mu=0.5, sd=0.4, alpha_vec=c(-1.25,1), beta_1=log(2), beta_vec=c(log(2),log(2)), lambda=0.001)

# Plot results of original time and time with error
sim1plot <- survdata(seed=123, n=1000000, error=1, p1=0.3, mu=0.5, sd=0.4, alpha_vec=c(-1.25,1), beta_1=log(2), beta_vec=c(log(2),log(2)), lambda=0.001)

# Plot results of original time and time with error
sim2plot <- survdata(seed=123, n=1000000, error=2, p1=0.3, mu=0.5, sd=0.4, alpha_vec=c(-1.25,1), beta_1=log(2), beta_vec=c(log(2),log(2)), lambda=0.001)

# Plot results of original time and time with error
sim3plot <- survdata(seed=123, n=1000000, error=3, p1=0.3, mu=0.5, sd=0.4, alpha_vec=c(-1.25,1), beta_1=log(2), beta_vec=c(log(2),log(2)), lambda=0.001)

# Plot results of original time and time with error
sim4plot <- survdata(seed=123, n=1000000, error=4, p1=0.3, mu=0.5, sd=0.4, alpha_vec=c(-1.25,1), beta_1=log(2), beta_vec=c(log(2),log(2)), lambda=0.001)
```

```{r surv_fig2, fig.width=12, fig.height=8, cache = TRUE, eval=FALSE}
#jpeg("surv_sim1_exp_lt_s.jpeg", units="in", width=12, height=8, res=300)
par(mfrow=c(2,3))

# surv0-2
plot(survfit(Surv(time_event, event) ~ z, data=sim0plot[[1]]), lty=c(1,1), col=1:2, xlim=c(0,300), 
     xlab="Time (days)", ylab="Survival Probability", main="Delayed event time: none", cex.main=1.8, cex.lab=1.5, cex.axis=1.2)

# surv1-2
plot(survfit(Surv(time_event, event) ~ z, data=sim1plot[[1]]), lty=c(1,1), col=1:2, xlim=c(0,300), 
     xlab="Time (days)", ylab="Survival Probability", main="Delayed event time: independent", cex.main=1.8, cex.lab=1.5, cex.axis=1.2)
lines(survfit(Surv(time_error, event_error) ~ z, data=sim1plot[[1]]), lty=c(1,1), col=3:4)

# surv2-2
plot(survfit(Surv(time_event, event) ~ z, data=sim2plot[[1]]), lty=c(1,1), col=1:2, xlim=c(0,300), 
     xlab="Time (days)", ylab="Survival Probability", main="Delayed event time: exposure", cex.main=1.8, cex.lab=1.5, cex.axis=1.2)
lines(survfit(Surv(time_error, event_error) ~ z, data=sim2plot[[1]]), lty=c(1,1), col=3:4)

# surv3-2
plot(survfit(Surv(time_event, event) ~ z, data=sim3plot[[1]]), lty=c(1,1), col=1:2, xlim=c(0,300), 
     xlab="Time (days)", ylab="Survival Probability", main="Delayed event time: confounder", cex.main=1.8, cex.lab=1.5, cex.axis=1.2)
lines(survfit(Surv(time_error, event_error) ~ z, data=sim3plot[[1]]), lty=c(1,1), col=3:4)

# surv4-2
plot(survfit(Surv(time_event, event) ~ z, data=sim4plot[[1]]), lty=c(1,1), col=1:2, xlim=c(0,300), 
     xlab="Time (days)", ylab="Survival Probability", main="Delayed event time: covariate", cex.main=1.8, cex.lab=1.5, cex.axis=1.2)
lines(survfit(Surv(time_error, event_error) ~ z, data=sim4plot[[1]]), lty=c(1,1), col=3:4)

# legend
plot(NULL, xaxt='n', yaxt='n', bty='n', ylab='', xlab='', xlim=0:1, ylim=0:1)
legend("center", c("True time K-M S(t|Contr)", "True time K-M S(t|Exposed)", "Error time K-M S(t|Contr)", "Error time K-M S(t|Exposed)"), title="Legend",
       lty=c(1,1,1,1), col=1:4, cex=1.6, bty="n")
#dev.off()
```

```{r}
result0_exp_s_lt <- result0
result1_exp_s_lt <- result1
result2_exp_s_lt <- result2
result3_exp_s_lt <- result3
result4_exp_s_lt <- result4
```
...
### Bias calculation for Cox Model 

```{r}
library(kableExtra)
load("~/Documents/Vanderbilt/Masters/Final Simulation Results/result_l_bias_df.Rds")
result_l_bias_df <- cbind(rep(c("No error", "Independent error", "Exposure-dependent error", "Confounder-dependent error", "Covariate-dependent error"), 12), result_l_bias_df)
colnames(result_l_bias_df) <- c("", "beta = 0", "beta = log(1.1)", "beta = log(1.15)", "beta = log(1.25)", "beta = log(1.5)", "beta = log(2)")
kable(result_l_bias_df) %>% kable_styling(full_width=TRUE) %>% 
  pack_rows("Exponential, left truncation", 1, 5) %>% 
  pack_rows("Log-normal, left truncation", 6, 10) %>% 
  pack_rows("Censoring (x), left truncation", 11, 15) %>% 
  pack_rows("Censoring (x & z), left truncation", 16, 20) %>% 
  pack_rows("Exponential, no left truncation", 21, 25) %>% 
  pack_rows("Log-normal, no left truncation", 26, 30) %>% 
  pack_rows("Censoring (x), no left truncation", 31, 35) %>% 
  pack_rows("Censoring (x & z), no left truncation", 36, 40) %>% 
  pack_rows("Exponential, Simulation 2", 41, 45) %>% 
  pack_rows("Log-normal, Simulation 2", 46, 50) %>% 
  pack_rows("Censoring (x), Simulation 2", 51, 55) %>% 
  pack_rows("Censoring (x & z), Simulation 2", 56, 60)
```